...
def preprocess_img(main_image):
    ...
    return main_image.resize((256, 256), Image.Resampling.LANCZOS)
...
class MarioEnvShim:
    ...
    def step(self, action):
        ...
        mario_action = 0
        # Right
        if action.get("DPAD_RIGHT") or (isinstance(action.get("AXIS_LEFTX"), (np.ndarray, list)) and action["AXIS_LEFTX"][0] > 16384):
            mario_action |= 1
        # Left
        if action.get("DPAD_LEFT") or (isinstance(action.get("AXIS_LEFTX"), (np.ndarray, list)) and action["AXIS_LEFTX"][0] < -16384):
            mario_action |= 2
        # Down
        if action.get("DPAD_DOWN") or (isinstance(action.get("AXIS_LEFTY"), (np.ndarray, list)) and action["AXIS_LEFTY"][0] > 16384):
            mario_action |= 4
        # Up
        if action.get("DPAD_UP") or (isinstance(action.get("AXIS_LEFTY"), (np.ndarray, list)) and action["AXIS_LEFTY"][0] < -16384):
            mario_action |= 8
        # Start
        if action.get("START"):
            mario_action |= 16
            ...
            while frames_count < max_frames:
                ...
                for i, a in enumerate(env_actions):
                    ...
                    for _ in range(action_downsample_ratio):
                        ...
                        # Resize for output videos
                        clean_viz = cv2.resize(obs_viz, (1920, 1080), interpolation=cv2.INTER_NEAREST)
                        debug_viz = create_viz(
                            cv2.resize(obs_viz, (1280, 720), interpolation=cv2.INTER_NEAREST),
                            i,
                            j_left,
                            j_right,
                            buttons,
